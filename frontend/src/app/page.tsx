...
export default function Auth() {
  ...
  useEffect(() => {
    // 1) Clear any previous inline GSAP props
    [
      titleRef.current,
      descRef.current,
      formRef.current,
    ...
    // 2) Fade‐slide in: title → subtitle → form
    ...
    // 4) Animate gradient on the Typewriter text
    ...
    if (descRef.current) {
      ...
      gradientTween = gsap.to(descRef.current, {
    ...
    // clean up on unmount or before next run
    return () => {
      tl.kill();
      blob1Tween.kill();
      blob2Tween.kill();
    ...
    >
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 h-full">
        ...
        <div className="flex items-center justify-center px-8">
          <div className="text-center md:text-left">
            ...
            <p
              ref={descRef}
              className="
                mt-4 text-lg md:text-xl font-medium tracking-wide min-h-[2.5rem]
                bg-gradient-to-r from-indigo-400 via-purple-400 to-pink-400
        ...
        <div className="flex items-center justify-center px-8">
          <div
            ref={formRef}
            className="w-full max-w-md bg-gray-800 bg-opacity-70 backdrop-blur-md rounded-xl shadow-xl p-8"
          >
